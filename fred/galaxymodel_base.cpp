...
class DFIntegrandNdim: public math::IFunctionNdim {
	...
	// 1. get the position/velocity components in cylindrical coordinates
	    ...
	    // 2. determine the actions
#ifdef SCATT   //See if low Lz orbit & if so scatter in Vxy plane
				    const double dL=.2;
				    ...
				    coord::PosVelCyl pv(posvel);
				    if(posvel.R*fabs(posvel.vphi)<3*DL){
					    double dV=DL/posvel.R;//change in vphi to give DL
					    pv.vphi+=2*(math::random()-.5)*dV;
					    pv.vR=sqrt(fmax(0,Vxy2-pow_2(pv.vphi)));
				    }
				    actions::Actions act = model.actFinder.actions(pv);
#else
...
class DFIntegrandAtPoint: public DFIntegrandNdim {
	...
	public:
		DFIntegrandAtPoint(const GalaxyModel& _model, bool separate,
				   ...
				   double bright = NULL, double faint = NULL, obs::los* _los = NULL)
				:
		    DFIntegrandNdim(_model, separate, bright, faint, _los),
...
public:
    ...
    :
        DFIntegrandNdim(_model, separate, bright),
        point(_point),
...
class DFIntegrandLOS: public DFIntegrandNdim {
	...
	public:
		DFIntegrandLOS(const GalaxyModel& _model, bool separate,	
			       const obs::los* _los, OperationMode _mode,
			       const double bright = NULL, const double faint= NULL)
				:
		    DFIntegrandNdim(_model, separate, bright, faint, _los),		//seperate means it does the model components seperately 
		    mode(_mode),			
		    numOutVal(1)		
		{
		//locate clostest distance r_p of los to GC
			const double cosb = los->cosb, cosl = los->cosl, sinb = los->sinb, sinl = los->sinl;			//watch units!! kpc
			double r_p = los->sun.xyz().x*cosb*cosl + los->sun.xyz().y*cosb*sinl
				     + los->sun.xyz().z*sinb;
			getVesc(los->Rzphi(r_p), model.potential, vesc, zeta);
		}

		    virtual coord::PosVelCyl unscaleVars(const double vars[], double* jac=0) const
		    {
			    double jacVel, s = unscaleDistance(vars[0],jac);// s in int units
...
EXP void computeMomentsLOS(const GalaxyModel& model, const obs::los* los,
    ...
    bool separate, const double reqRelError, const int maxNumEval,const double bright, const double faint)
{
    ...
    double xupper[4] = {1e-2, 1e-2, 1e-2, 1e-2};
...
EXP void computeMoments(const GalaxyModel& model, const coord::PosCyl& point,
			double* density,    double* velocityFirstMoment,    coord::Vel2Cyl* velocitySecondMoment,
			const units::InternalUnits& intUnits, // added for sKpc
			...
			const double bright, const double faint, obs::solarShifter* sun)
{
    ...
    obs::los Los(toPosCar(point),sun != NULL? sun->xyz() : coord::PosCar(0,0,0), intUnits);// added for sKpc
    obs::los* ptrLos=NULL;
    if(sun != NULL) ptrLos = &Los; 
    DFIntegrandAtPoint fnc(model, separate, point, mode, bright, faint, ptrLos);
					      ...
					      obs::los* Los)
{
	...
	DFIntegrandAtPoint fnc(model, false, loc, mode, bright, faint, Los);
...
EXP std::vector<coord::VelCylMag> sampleVelocityLF(const GalaxyModel& model,
					      ...
					      obs::los* Los)
{
	DF_LFIntegrandAtPoint fnc(model, loc, bright, faint, Los);
	math::Matrix<double> result;      // sampled scaled velocities
	double totalDens, errorDens;      // total normalization of the DF and its estimated error
	double xlower[4] = {0,0,0,1}; // boundaries of sampling region in scaled coordinates
	double xupper[4] = {1,1,1,1};
	math::sampleNdim(fnc, xlower, xupper, numSamples, result, NULL, &totalDens, &errorDens);
	const double pointDens = totalDens / result.rows();
...
EXP std::vector<coord::VelCylMag> sampleVelocity_LF(const GalaxyModel& model,
					      ...
					      obs::los* Los)
{
	std::vector<coord::VelCyl> vels=sampleVelocity(model, loc, numSamples, bright, faint, Los);
	double s = Los->s(loc);
...
EXP std::vector<coord::PosVelCyl> sampleLOS(const GalaxyModel& model,
					    const obs::los* Los, const size_t numSamples,
					    const double bright, const double faint)
{
	double *velocityMoment=NULL;
	OperationMode mode = static_cast<OperationMode>(0);
	DFIntegrandLOS fnc(model, false, Los, mode, bright, faint);			//DFIntegrandLOS will need changing instead of loc we need (l,b) 
//fnc needs to give a prob dist in v and s(los distance) instead of just v like integrandatpoint
...
EXP std::vector<coord::PosVelCylMag> sampleLOSLF(const GalaxyModel& model,
					    ...
					    const double bright, const double faint)
{
	DF_LFIntegrandLOS fnc(model, Los, bright, faint);			//DFIntegrandLOS will need changing instead of loc we need (l,b) 
//fnc needs to give a prob dist in v and s(los distance) instead of just v like integrandatpoint
	math::Matrix<double> result;  // sampled scaled distances & velocities
	double totalDens, errorDens;  // total normalization of the DF and its estimated error
