...
template<> double scale(const ScalingCos2& scaling, double u) {
...
template<> double unscale(const ScalingCos2& scaling, double s, double* duds) {
	if(duds)
...
// u in [uleft, uright], cubic transformation 
template<> double scale(const ScalingSin2& scaling, double u) {
	if(u == scaling.uleft)  return 0;
	if(u == scaling.uright) return 1;
	double Du = .5*(scaling.uright-scaling.uleft);
	double s = asin(sqrt((u-scaling.uleft)*(scaling.uright-u)/pow_2(Du)))/M_PI;
	if(u < .5*(scaling.uright+scaling.uleft)) return s;
	else return 1-s;
}

template<> double unscale(const ScalingSin2& scaling, double s, double* duds) {
	double ubar = .5*(scaling.uright+scaling.uleft), Du = .5*(scaling.uright-scaling.uleft);
	if(duds)
		*duds = M_PI*Du*sin(s*M_PI);
	return ubar-Du*cos(s*M_PI);
}
