...
void printit(math::ScaledIntegrand<math::ScalingCub>& f,double x0, double x1){
	...
	const int N=50;
	for(int i=0;i<N;i++){
		double x=x0+i*(x1-x0)/(double)(N-1);
		fprintf(ofile,"%g %g\n",x,f.value(x));
	}
	fclose(ofile);
}
...
double FDfinder::bestFD(double* umin){
...
*/
EXP AxisymIntLimits findIntegrationLimitsAxisym(const AxisymFunctionBase& fnc)
{
    AxisymIntLimits lim;
    ...
    if(!isFinite(lim.lambda_min)) {  // not yet determined
	    if(lim.nu_max<0.7*delta) //not a highly inclined orbit
		    lim.lambda_min = math::findRoot(fnc, lambda_lower, lambda_pos, ACCURACY_RANGE);
	    else {//highly inclined orbit so there could be a false region of p2>0 at small lambda
		    double dlambda = lambda_pos -delta - (df_lambda
			    - sqrt(pow_2(df_lambda) - 2*f_lambda*d2f_lambda))/d2f_lambda;
		    if(dlambda > 0){
			    double ftry = fnc(delta + dlambda);
			    int knt = 0;
			    while(ftry>0 && knt < 20){
				    dlambda *= 0.8;
				    ftry = fnc(delta+dlambda);
				    knt++;
			    }
			    if(knt == 20)
				    utils::msg(utils::VL_WARNING, "findIntegrationLimitsAxisym",
					       "failed at lambda="+utils::toString(fnc.point.lambda)+
					       ", nu="+utils::toString(fnc.point.nu)+", E="+
					       utils::toString(fnc.E)+", Lz="+utils::toString(fnc.Lz)+", I3="+utils::toString(fnc.I3));
			    lim.lambda_min = math::findRoot(fnc, delta+dlambda, lambda_pos, ACCURACY_RANGE);
		    }
		    if(!isFinite(lim.lambda_min)){//still not chosen
			    double dlambda = (lambda_pos - delta)/10.;
			    int knt = 1;
			    double ftry = fnc(lambda_pos - knt*dlambda);
			    while(ftry>0 && knt<10){
				    knt++;
				    ftry = fnc(lambda_pos - knt*dlambda);
			    }
			    lim.lambda_min = math::findRoot(fnc, lambda_pos - knt*dlambda, lambda_pos, ACCURACY_RANGE);
		    }
	    }
    }
    ...
    const AxisymFunctionBase& fnc, const AxisymIntLimits& lim)
{
    ...
    printit(transf_l,0,yl); printit(transf_n,0,yn);
    // derivatives w.r.t. I3
    integrand.a = AxisymIntegrand::aminus1;
    integrand.c = AxisymIntegrand::cminus1;
    der.dSdI3 = 
...
#pragma omp parallel for schedule(dynamic)
    ...
    for(int iEL = 0; iEL < sizeEL; iEL++) {
	    ...
	    for(int iI = 0; iI < sizeI; iI++){
		    ...
		    double vR = sqrt(vmer2 - 2*gridI[iI]*I3max/lambda);
		    FDfinder state(E,Rsh,vR,FD,pot);
		    grid3dD[index] = fmin(FD, state.bestFD());
	    }
        }
    ...
    invPhi0(1./_pot->value(coord::PosCyl(0,0,0))), pot(_pot), interp(*pot)
{
    ...
    createGridFocalDistance(pot, gridE, gridL, gridI, /*output*/ grid3dD, grid2dR);
