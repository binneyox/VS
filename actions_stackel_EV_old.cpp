    ...
    const AxisymFunctionBase& fnc, const AxisymIntLimits& lim)
{
    ...
    FILE *log_f; fopen_s(&log_f,"stackel.log","a");
    for(int i=0;i<100;i++){
	    double lam=lim.lambda_min+1e-5+i*(lim.lambda_max-lim.lambda_min-2e-5)/99.;
	    fprintf(log_f,"%g %g\n",lam,fnc(lam));
    }
...
//    fprintf(log_f,"%g %g %g %g %g %g\n",der.dJrdE,der.dJzdE,der.dJrdI3,der.dJzdI3,der.dJrdLz,der.dJzdLz);
    fprintf(log_f,"%g %g %g %g %g %g\n",lim.lambda_min,lim.lambda_max,lim.nu_min,lim.nu_max,
	      der.dJrdLz,der.dJzdLz);
    fclose(log_f);
    return der;
}
