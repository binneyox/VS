...
void Cheby::chebyfit(double * x, double * y, const int np, const int NC)  
{
  ...
  double *bin=new double[NChb];
  double *s11=new double[NChb+1];
  double *ain=new double[NChb+1];
  svector crow((np-2),0);
  svector wrow = {0,0}; 
  smatrix coeff,work;
  for(int i=0;i<3;i++) coeff.push_back(crow); // null the matrix
  for(int i=0;i<NChb+2;i++) work.push_back(wrow); // null the matrix
//  WD::Matrix<double,3,np-2> coeff;
//  WD::Matrix<double,NChb+2,2> work;
  double alpha = 2./(x[np-1]-x[0]),
    beta = -(x[np-1]+x[0])/(x[np-1]-x[0]),  // can be done quicker
    Q = alpha/beta,dydx, 
    //xtmp[np],coeff[3][np-2],bin[NChb],s11[NChb+1],work[NChb+2][2],
