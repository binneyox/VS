...
// and its derivatives w.r.t. th
    ...
    A[0][0] = scq[0]*pow_2(dQPdt[0][0]) + scq[1]*pow_2(dQPdt[1][0]) 
      + scq[2]*pow_2(dQPdt[2][0]) + scq[3]*pow_2(dQPdt[3][0]);
    A[0][1] = scq[0]*dQPdt[0][0]*dQPdt[0][1] + scq[1]*dQPdt[1][0]*dQPdt[1][1] 
      + scq[2]*dQPdt[2][0]*dQPdt[2][1] + scq[3]*dQPdt[3][0]*dQPdt[3][1];
    A[1][0] = A[0][1];
    A[1][1] = scq[0]*pow_2(dQPdt[0][1]) + scq[1]*pow_2(dQPdt[1][1])
      + scq[2]*pow_2(dQPdt[2][1]) + scq[3]*pow_2(dQPdt[3][1]);
}
...
void Torus::CheckLevCof(PSPD QP_aim, Angles A_in) {
  ...
  DB2    B;
  double tmpx2 = pow_2(Rmax-Rmin) + pow_2(zmax), tmpv2;
...
// [R-R(th)]^2 + [z-z(th)]^2 + scale^2 * ( [vR-vR(th)]^2 + [vz-vz(th)]^2 )
{
  ...
  if(scale==0.) {
    //cerr << Rmin << ' ' << Rmax << ' ' << zmax << '\n';
    double tmpx2 = pow_2(0.5*(Rmax-Rmin)) + pow_2(zmax), tmpv2;
    Angles tmpA = 0.; tmpA[0] = Pih;
    PSPD   tmpqp = MapfromToy(tmpA);
...
// (scale[2]*[vR-vR(th)])^2 + (scale[3]*[vz-vz(th)])^2 
{
  ...
  if(scales==0.) {
    ...
    Angles tmpA = 0.; tmpA[0] = Pih;
    PSPD   tmpqp = MapfromToy(tmpA);
    double tmpR2 = pow_2(0.5*(Rmax-Rmin)), tmpz2 = zmax*zmax, 
  ...
  for(int i=0;i!=ngridr;i++) {
    ...
    for(int j=0;j!=ngridz;j++) {
      Atest[1] = TPi*j/double(ngridz);
      QP = MapfromToy(Atest);
      chi = sc[0]*sc[0]*pow_2(QP(0)-QP_aim(0)) + 
	sc[1]*sc[1]*pow_2(QP(1)-QP_aim(1)) +
	sc[2]*sc[2]*pow_2(QP(2)-QP_aim(2)) +
	sc[3]*sc[3]*pow_2(QP(3)-QP_aim(3));
      if(chi<chio) {
	Astart = Atest;
	chio = chi;
      }
...
void Torus::AutoTorus(Potential * Phi, const Actions Jin, const double R0) {
  ...
  IP[0] = sqrt(dPdR*atmp*pow_2(btmp+atmp)/Rc);
  IP[2] = fabs(Jin(2));
  GenPar SN;
  SN.MakeGeneric();
  GenPar s1(SN);
