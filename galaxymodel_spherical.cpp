...
EXP particles::ParticleArraySph samplePosVel(
...
EXP std::vector<double> computeDensity(const math::IFunction& df, const potential::PhaseVolume& pv,
...
EXP void computeProjectedDensity(const math::IFunction& dens, const math::IFunction& velDisp,
...
EXP void getAsymptoticSlopes(const math::IFunction& f, const std::vector<double>& gridh,
...
EXP SphericalIsotropicModel::SphericalIsotropicModel(const potential::PhaseVolume& _phasevol,
...
EXP SphericalIsotropicModelLocal::SphericalIsotropicModelLocal(
...
EXP void SphericalIsotropicModelLocal::evalLocal(
...
EXP double SphericalIsotropicModelLocal::sampleVelocity(double Phi, math::PRNGState* state) const
{
    if(!(Phi<0))
...
EXP double SphericalIsotropicModelLocal::density(double Phi) const
{
    if(!(Phi<0))
        throw std::invalid_argument("SphericalIsotropicModelLocal: invalid value of Phi");
    double hPhi     = phasevol(Phi);
...
EXP double SphericalIsotropicModelLocal::velDisp(double Phi) const
{
    if(!(Phi<0))
        throw std::invalid_argument("SphericalIsotropicModelLocal: invalid value of Phi");
    double hPhi     = phasevol(Phi);
...
EXP void difCoefEnergy(const SphericalIsotropicModel& model, double E, double m,
...
EXP double difCoefLosscone(const SphericalIsotropicModel& model, const math::IFunction& pot, double E)
{
...
EXP void writeSphericalIsotropicModel(
