    ...
    const char* fncName, const char* arrName)
{
    for(size_t i=0, size=arr.size(); i<size; i++) {
        if(!isFinite(arr[i]))
		throw std::length_error(std::string(fncName) + ": " + arrName +
    ...
    const char* fncName, const char* arrName)
{
    for(size_t i=0, size=arr.size(); i<size; i++)
        if(!isFinite(arr[i]))
		std::cout << std::string(fncName) + ": " + std::string(arrName) +
...
EXP BaseInterpolator1d::BaseInterpolator1d(
...
EXP LinearInterpolator::LinearInterpolator(
    const std::vector<double>& xvalues, const std::vector<double>& fvalues)
:
...
EXP void LinearInterpolator::evalDeriv(const double x, double* value, double* deriv, double* deriv2) const
{
...
EXP double LinearInterpolator::evalReverse(const double f) const
{
	int i = std::max<int>(0, std::min<int>(fval.size()-2, binSearch(f, &fval[0], fval.size())));
	return linearInterp(f, fval[i], fval[i+1], xval[i], xval[i+1]);
}

//-------------- CUBIC SPLINE --------------//

// construct a natural or clamped cubic spline
EXP CubicSpline::CubicSpline(
    const std::vector<double>& xvalues, const std::vector<double>& fvalues,
...
EXP double CubicSpline::integrate(double x1, double x2, int n) const {
    return integrate(x1, x2, Monomial(n));
}

EXP double CubicSpline::integrate(double x1, double x2, const IFunctionIntegral& f) const
{
    if(x1==x2)
        return 0;
    if(x1>x2)
...
EXP QuinticSpline::QuinticSpline(
    const std::vector<double>& xvalues, const std::vector<double>& fvalues,
    const std::vector<double>& fderivs)
:
    BaseInterpolator1d(xvalues, fvalues), fder(fderivs)
...
EXP void QuinticSpline::evalDeriv(const double x, double* val, double* deriv, double* deriv2) const
{
    int size = xval.size();
    if(size == 0)
        throw std::length_error("Empty spline");
...
EXP void LogLogSpline::evalDeriv(const double x, double* value, double* deriv, double* deriv2) const
{
    int size = xval.size();
    if(size == 0)
        throw std::length_error("Empty spline");
...
template<int N>
EXP BsplineInterpolator1d<N>::BsplineInterpolator1d(const std::vector<double>& xvalues) :
...
template<int N>
EXP unsigned int BsplineInterpolator1d<N>::nonzeroComponents(
...
template<int N>
EXP double BsplineInterpolator1d<N>::interpolate(
...
template<int N>
EXP void BsplineInterpolator1d<N>::eval(const double* x, double values[]) const
{
...
template<int N>
EXP void BsplineInterpolator1d<N>::addPoint(const double* x, double mult, double values[]) const
{
    double bspl[N+1];
    unsigned int leftInd = bsplineValues<N>(*x, &xval[0], xval.size(), bspl);
    for(int i=0; i<=N; i++)
...
template<int N>
EXP std::vector<double> BsplineInterpolator1d<N>::deriv(const std::vector<double> &amplitudes) const
{
    if(amplitudes.size() != numComp)
...
template<int N>
EXP std::vector<double> BsplineInterpolator1d<N>::antideriv(const std::vector<double> &amplitudes) const
{
    if(amplitudes.size() != numComp)
...
template<int N>
EXP double BsplineInterpolator1d<N>::integrate(double x1, double x2,
...
template class EXP BsplineInterpolator1d<0>;
template class EXP BsplineInterpolator1d<1>;
template class EXP BsplineInterpolator1d<2>;
template class EXP BsplineInterpolator1d<3>;


// ------ Finite-element features of B-splines ------ //

template<int N>
EXP void FiniteElement1d<N>::setup()
{
...
template<int N>
EXP std::vector<double> FiniteElement1d<N>::computeProjVector(
...
template<int N>
EXP BandMatrix<double> FiniteElement1d<N>::computeProjMatrix(
...
void FiniteElement1d<N>::addPointConv(const double x, double mult,
					  const IFunctionIntegral& kernel,
					  unsigned int derivOrder, //output
					  double values[]) const
{
    const unsigned int
    gridSize    = integrNodes.size(),  // # of points in the integration grid
...
template<int N>
EXP std::vector<double> FiniteElement1d<N>::computeConvVector(
...
template<int N>
EXP Matrix<double> FiniteElement1d<N>::computeConvMatrix(const IFunctionIntegral& kernel,
...
template<int N>
EXP std::vector<double> FiniteElement1d<N>::computeAmplitudes(const IFunction& F) const
{
...
// force template instantiations for several values of N
template class EXP FiniteElement1d<0>;
template class EXP FiniteElement1d<1>;
template class EXP FiniteElement1d<2>;
template class EXP FiniteElement1d<3>;


// ------ INTERPOLATION IN 2D ------ //

EXP BaseInterpolator2d::BaseInterpolator2d(
...
EXP void LinearInterpolator2d::evalDeriv(const double x, const double y,
...
EXP CubicSpline2d::CubicSpline2d(
    const std::vector<double>& xvalues,
    const std::vector<double>& yvalues,
    const IMatrixDense<double>& fvalues,
    bool regularize,
...
EXP void CubicSpline2d::evalDeriv(const double x, const double y,
    double *z, double *z_x, double *z_y, double *z_xx, double *z_xy, double *z_yy) const
{
    if(fval.empty())
...
EXP QuinticSpline2d::QuinticSpline2d(
    const std::vector<double>& xvalues,
    const std::vector<double>& yvalues,
    const IMatrixDense<double>& fvalues,
...
EXP void QuinticSpline2d::setupWoutMixedDeriv(const size_t xsize, const size_t ysize)
{
...
EXP void QuinticSpline2d::setupWithMixedDeriv(const size_t xsize, const size_t ysize)
{
    // temporary arrays for 1d splines
...
EXP void QuinticSpline2d::evalDeriv(const double x, const double y,
...
EXP LinearInterpolator3d::LinearInterpolator3d(
    const std::vector<double>& xvalues,
    const std::vector<double>& yvalues,
...
EXP double LinearInterpolator3d::value(double x, double y, double z) const
{
    const int
    nx = xval.size(),
    ny = yval.size(),
...
EXP CubicSpline3d::CubicSpline3d(
    const std::vector<double>& xvalues,
    const std::vector<double>& yvalues,
    const std::vector<double>& zvalues,
    const std::vector<double>& fvalues,
...
EXP double CubicSpline3d::value(const double x, const double y, const double z) const
{
    const int
    nx = xval.size(),
    ny = yval.size(),
...
template<int N>
EXP BsplineInterpolator3d<N>::BsplineInterpolator3d(
    const std::vector<double>& xvalues,
    const std::vector<double>& yvalues,
    const std::vector<double>& zvalues)
:
...
template<int N>
EXP void BsplineInterpolator3d<N>::nonzeroComponents(const double point[3],
...
template<int N>
EXP double BsplineInterpolator3d<N>::interpolate(
...
template<int N>
EXP void BsplineInterpolator3d<N>::eval(const double point[3], double values[]) const
{
    unsigned int leftInd[3];
    double weights[(N+1)*(N+1)*(N+1)];
    nonzeroComponents(point, leftInd, weights);
...
template<int N>
EXP double BsplineInterpolator3d<N>::valueOfComponent(const double point[3], unsigned int indComp) const
{
    if(indComp>=numComp)
...
template<int N>
EXP void BsplineInterpolator3d<N>::nonzeroDomain(unsigned int indComp,
...
template<int N>
EXP SparseMatrix<double> BsplineInterpolator3d<N>::computeRoughnessPenaltyMatrix() const
{
...
template<int N>
EXP std::vector<double> createBsplineInterpolator3dArray(const IFunctionNdim& F,
    const std::vector<double>& xvalues,
    const std::vector<double>& yvalues,
    const std::vector<double>& zvalues)
{
...
template<int N>
EXP std::vector<double> createBsplineInterpolator3dArrayFromSamples(
...
template class EXP BsplineInterpolator3d<1>;
template class EXP BsplineInterpolator3d<3>;

template EXP std::vector<double> createBsplineInterpolator3dArray<1>(const IFunctionNdim& F,
    const std::vector<double>&, const std::vector<double>&, const std::vector<double>&);
template EXP std::vector<double> createBsplineInterpolator3dArray<3>(const IFunctionNdim& F,
    const std::vector<double>&, const std::vector<double>&, const std::vector<double>&);
template EXP std::vector<double> createBsplineInterpolator3dArrayFromSamples<1>(
    const IMatrixDense<double>&, const std::vector<double>&,
    const std::vector<double>&, const std::vector<double>&, const std::vector<double>&);
template EXP std::vector<double> createBsplineInterpolator3dArrayFromSamples<3>(
    const IMatrixDense<double>&, const std::vector<double>&,
    const std::vector<double>&, const std::vector<double>&, const std::vector<double>&);
...
EXP SplineApprox::Impl::Impl(const std::vector<double> &_knots,
...
EXP SplineApprox::Impl::FitData SplineApprox::Impl::initFit(const std::vector<double> &yvalues) const
{
...
EXP void SplineApprox::Impl::computeAmplitudes(const FitData &fitData, double lambda,
...
EXP void SplineApprox::Impl::solveForAmplitudesWithEDF(const std::vector<double> &yvalues, double EDF,
    std::vector<double> &ampl, double &RSS) const
{
    if(EDF==0)
        EDF = numKnots;
...
EXP void SplineApprox::Impl::solveForAmplitudesWithAIC(const std::vector<double> &yvalues, double deltaAIC,
    std::vector<double> &ampl, double &RSS, double &EDF) const
{
    if(deltaAIC < 0)
...
EXP SplineApprox::SplineApprox(const std::vector<double> &grid,
    const std::vector<double> &xvalues, const std::vector<double> &weights) :
    impl(new Impl(grid, xvalues, weights)) {}

EXP SplineApprox::~SplineApprox()
{
    delete impl;
}

EXP std::vector<double> SplineApprox::fit(
...
EXP std::vector<double> SplineApprox::fitOversmooth(
...
template<int N>
EXP std::vector<double> splineLogDensity(const std::vector<double> &grid,
    const std::vector<double> &xvalues, const std::vector<double> &weights,
...
// force the template instantiations to compile
template EXP std::vector<double> splineLogDensity<1>(
    const std::vector<double>&, const std::vector<double>&, const std::vector<double>&, FitOptions, double);
template EXP std::vector<double> splineLogDensity<3>(
    const std::vector<double>&, const std::vector<double>&, const std::vector<double>&, FitOptions, double);

//------------ GENERATION OF UNEQUALLY SPACED GRIDS ------------//

EXP std::vector<double> createUniformGrid(unsigned int nnodes, double xmin, double xmax)
{
...
EXP std::vector<double> createExpGrid(unsigned int nnodes, double xmin, double xmax)
{
...
EXP std::vector<double> createNonuniformGrid(unsigned int nnodes, double xmin, double xmax, bool zeroelem)
...
EXP std::vector<double> mirrorGrid(const std::vector<double> &input)
{
...
EXP std::vector<double> createSymmetricGrid(unsigned int nnodes, double xmin, double xmax)
{
    if(isFinite(xmax)) {
        if(nnodes%2 == 1)
...
EXP std::vector<double> createAlmostUniformGrid(unsigned int gridsize,
...
EXP std::vector<double> createInterpolationGrid(const IFunction& fnc, double eps)
{
