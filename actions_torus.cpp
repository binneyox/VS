...
coord::PosVelCyl ActionMapperTorus::map(const Angles& Ang, Frequencies* freq) const{
	actions::Actions J(torus->action(0),torus->action(0),torus->action(0));
	actions::ActionAngles actAng(J,Ang);
	if(freq)
		return ActionMapperTorus::map(actAng, freq);
	else
		return ActionMapperTorus::map(actAng);
}
