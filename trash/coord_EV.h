...
struct EXP UVSph{
	const double Delta, Delta2;     ///< Delta^2 = gamma - alpha > 0
	UVSph(double);                  ///< Delta is the focal distance
...
template<> struct EXP VelT<ProlMod> {
	double prho, ptau, pphi;
	VelT<ProlMod>() {};
	VelT<ProlMod>(double _prho, double _ptau, double _pphi) : prho(_prho), ptau(_ptau), pphi(_pphi) {};
};
typedef struct EXP VelT<ProlMod> VelProlMod;

/// momentum in UV prolate spheroidal coordinates, canonically conjugate to the position
template<> struct EXP VelT<UVSph> {
	double pu, pv, pphi;
	VelT<UVSph>() {};
	VelT<UVSph>(double _pu, double _pv, double _pphi) : pu(_pu), pv(_pv), pphi(_pphi) {};
};
typedef struct EXP VelT<UVSph> VelUVSph;
...
template<> struct EXP PosVelT<ProlMod>: public PosProlMod, public VelProlMod {
    PosVelT<ProlMod>(const PosProlMod& pos, const VelProlMod& vel) : PosProlMod(pos), VelProlMod(vel) {}
};
typedef struct EXP PosVelT<ProlMod> PosVelProlMod;
