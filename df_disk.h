...
struct ExponentialParam: public AgeVelocityDispersionParam{
	...
	ExponentialParam() :  ///< set default values for all fields
	    norm(NAN), mass(NAN), Jr0(NAN), Jz0(NAN), Jphi0(NAN), pr(0.5), pz(0.5), addJden(10), addJvel(100) {}
};
...
/// taperExp DF
/// Same as Exponential DF except with an inner taper set at Jphi =
/// Jtaper with transition over Jtran

/// Parameters that describe the taperExp distribution function
struct taperExpParam{
	double
			...
			Jr0,        ///< scale action setting the radial velocity dispersion
			Jz0,        ///< scale action setting the disk thickness and the vertical velocity dispersion
			Jtaper,	    ///< Jphi inside which DF more than halved
			Jtrans,     ///< width of transition around Jtaper
			Jcut,	    ///< Jphi outside which DF is more than halved
			Delta,     ///< width of transition around Jcut
			Jphi0,      ///< scale action setting the disk radius
			addJden,    ///< additional contribution to the sum of actions that affects the density profile
			pr,	    ///< power of radial variation of sigR
			pz,	    ///< power of radial variation of sigz
	...
	taperExpParam() : ///< set default values for all fields
	    norm(NAN), mass(NAN), Jr0(NAN), Jz0(NAN), Jtaper(NAN), Jtrans(-1),
	    Jcut(NAN), Delta(-1), Jphi0(NAN), addJden(0), pr(0.5), pz(0.5), addJvel(0) {}
};

class EXP taperExp: public df::BaseDistributionFunction{
