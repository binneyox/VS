...
EXP actions::Actions ActionSpaceScalingTriangLog::toActions(const double vars[3], double* jac) const
{
...
EXP void ActionSpaceScalingTriangLog::toScaled(const actions::Actions &acts, double vars[3]) const
{
...
EXP ActionSpaceScalingRect::ActionSpaceScalingRect(double _scaleJm, double _scaleJphi) :
...
EXP actions::Actions ActionSpaceScalingRect::toActions(const double vars[3], double *jac) const
{
...
EXP void ActionSpaceScalingRect::toScaled(const actions::Actions &acts, double vars[3]) const
{
    if(!(acts.Jr>=0 && acts.Jz>=0 && acts.Jphi==acts.Jphi))
        throw std::range_error("ActionSpaceScaling: input actions out of range");
...
class EXP DFIntegrandNdim: public math::IFunctionNdim {
public:
...
EXP double BaseDistributionFunction::totalMass(const double reqRelError, const int maxNumEval,
...
//Returns the fraction of a population that lies between Bright & Faint
EXP double BaseDistributionFunction::fraction(double Bright,double Faint) const{
	if(Bright>Faint) return 0;
	else{
		double f1,f2;
		interps.evalDeriv(Bright,&f1);
		interps.evalDeriv(Faint, &f2);
		return f2 - f1;
	}
}

//Evaluates the luminosity funcion
EXP double BaseDistributionFunction::LF(double mag) const{
	double df;
	interps.evalDeriv(mag, NULL, &df);
	return df;
}

//selects absM between Bright and Faint according to LF
EXP double BaseDistributionFunction::selectMag(const double Bright,const double Faint) const{
	double fB,fF;
	interps.evalDeriv(Bright,&fB);
	interps.evalDeriv(Faint,&fF);
	double fw=fF-math::random(NULL)*(fF-fB);
	return interps.evalReverse(fw);
}

EXP void BaseDistributionFunction::readBrighterThan(std::string Fname){
	std::ifstream infile(Fname);
	if(!infile.good())
		std::cout << "No fraction file " << Fname << "\n";
	else{
//		std::cout << "PopFile "+Fname+" ok" << "\n";
		FILE* ifile;
		fopen_s(&ifile,Fname.c_str(),"r");
//	    std::fstream ifile(par.Fname);
		std::vector<double> mags;
		std::vector<double> fracs;
		double x,y;			    
		while(!feof(ifile)){
			if(2!=fscanf_s(ifile,"%lf %lf",&x,&y)) break;
			mags.push_back(x);
			fracs.push_back(y);
		}
		fclose(ifile);
		interps = math::LinearInterpolator(mags,fracs);
	}
}

EXP double totalEntropy(const BaseDistributionFunction& DF, const double reqRelError, const int maxNumEval)
{
...
EXP std::vector<actions::Actions> sampleActions(const BaseDistributionFunction& DF, const size_t numSamples,
    double* totalMass, double* totalMassErr)
{
    double xlower[3] = {0, 0, 0};  // boundaries of integration region in scaled coordinates
    double xupper[3] = {1, 1, 1};
