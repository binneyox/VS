...
*/
EXP void sphHarmArray(const unsigned int lmax, const unsigned int m, const double tau,
...
*/
EXP void trigMultiAngle(const double phi, const unsigned int m, const bool needSine, double* outputArray);
...
*/
EXP SphHarmIndices getIndicesFromCoefs(const std::vector<double> &C);

/** Collect the list of azimuthal harmonic indices (m) that are non-zero
    for the given symmetry type and expansion order */
EXP std::vector<int> getIndicesAzimuthal(int mmax, coord::SymmetryType sym);
...
*/
class EXP SphHarmTransformForward {
public:
...
*/
EXP double sphHarmTransformInverse(const SphHarmIndices& ind, const double coefs[],
