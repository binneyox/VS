...
namespace internal {  // internal routines

void printitS(math::ScaledIntegrand<math::ScalingSin2>& f,double x0, double x1){
	FILE *ofile; int err=fopen_s(&ofile,"intgrnds.dat","a");
	if(err){
		printf("no file intgrnds.dat\n"); return;
	}
	const int N=50;
	double dx=x1-x0;
	for(int i=0;i<N;i++){
		double x=x0+1.e-6*dx+i*(1-2.e-6)*dx/(double)(N-1);
		fprintf(ofile,"%g %g\n",x,f.value(x));
	}
	fclose(ofile);
}
void printitC(math::ScaledIntegrand<math::ScalingCos2>& f,double x0, double x1){
	FILE *ofile; int err=fopen_s(&ofile,"intgrnds.dat","a");
	if(err){
		printf("no file intgrnds.dat\n"); return;
	}
...
void printitSI(math::ScaledIntegrand<math::ScalingSin2Inv>& f,double x0, double x1){
	FILE *ofile; int err=fopen_s(&ofile,"intgrnds.dat","a");
	if(err){
		printf("no file intgrnds.dat\n"); return;
	}
...
void printitCI(math::ScaledIntegrand<math::ScalingCos2Inv>& f,double x0, double x1){
	FILE *ofile; int err=fopen_s(&ofile,"intgrnds.dat","a");
	if(err){
		printf("no file intgrnds.dat\n"); return;
	}
...
struct AxisymIntLimits {
    ...
    int l_integrOrder;   ///< order of GL quadrature that depends on the eccentricity
    int n_integrOrder;   ///< order of GL quadrature that depends on the eccentricity
};
...
void print_dJ(AxisymActionDerivatives& dJ){
	FILE *ofile; int err=fopen_s(&ofile,"ders.dat","a");
	if(err){
		printf("no file ders.dat\n"); return;
	}
	fprintf(ofile,"%f %f %f %f %f %f\n",dJ.dJrdE, dJ.dJrdI3,
		  dJ.dJrdLz, dJ.dJzdE, dJ.dJzdI3, dJ.dJzdLz);
	fclose(ofile);
}
void print_der(AxisymIntDerivatives der){
	FILE *ofile; int err=fopen_s(&ofile,"ders.dat","a");
	if(err){
		printf("no file ders.dat\n"); return;
	}
	fprintf(ofile,"%f %f %f %f %f %f\n",der.Omegar,der.Omegaz,der.Omegaphi,
		  der.dI3dJr, der.dI3dJz, der.dI3dJphi);
	fclose(ofile);
}
...
/** compute true (E, Lz) and approximate (Ilambda, Inu) integrals of motion in an arbitrary 
    ...
    together with the coordinates in its prolate spheroidal coordinate system 
*/
EXP AxisymFunctionFudge findIntegralsOfMotionAxisymFudge(
...
// with Lz=0. Rsh is the radius of the shell orbit and vR is
// what's required to create the orbit of interest shooting from
// (Rsh,0). Delta0 is an estimate of FD; we start from a much smaller
// value. The method bestFD() returns the
// FD and optionally as an argument the value of u at which p_u^2=0
//
class EXP FDfinder{
	private:
		double E,Rsh,vR,P0,Dstart,EmP0;
		const potential::PtrPotential pot;
	public:
		FDfinder(const double _E,const double _Rsh,const double _vR,
			 const double _Delta0,
			 const potential::PtrPotential _pot) :
		    E(_E), Rsh(_Rsh), vR(_vR), Dstart(.3*_Delta0), pot(_pot){
			P0=pot->value(coord::PosCyl(Rsh,0,0));
			EmP0=E-P0;
		}
		math::Matrix<double> derivs(double u,double Delta,double* p2=NULL,double* p2prime=NULL);
		double bestFD(double* = NULL);
};
...
//implements N-R search for p_u^2=dp_u^2/du=0 to return FD
double FDfinder::bestFD(double* lambda_dip){
	double u=asinh(.8*Rsh/Dstart), u0=asinh(Rsh/Dstart), pu0=vR*Dstart*cosh(u0);
	double p2=1,p2prime=1,Delta=Dstart,fac=1;
	if(Dstart==0) return 0;
	int i=0, imax=30;
	while(fabs(p2)+fabs(p2prime)>.001 && i<imax){
	...
	if(lambda_dip != NULL) *lambda_dip = pow_2(Delta * cosh(u));
	if(i<imax) return Delta;
	else{
		return Dstart/.3;
	}
}	
...
*/
EXP AxisymIntLimits findIntegrationLimitsAxisym(const AxisymFunctionBase& fnc, double lambda_dip)
{
    AxisymIntLimits lim;
...
//    double lambda_lower = delta; // lower bound on the interval for lambda_min
    double lambda_lower; // lower bound on the interval for lambda_min
    if(!isFinite(lambda_dip)) lambda_lower = delta;
    else lambda_lower =  fnc(lambda_dip)<=0?  lambda_dip : delta;
    ...
    lim.l_integrOrder = integrOrder(RperiOverRapo);
#ifdef SIN2
    lim.n_integrOrder = integrOrder(1-lim.nu_max/delta);
#else
    lim.n_integrOrder = lim.l_integrOrder;
#endif
    return lim;
}
    ...
    const AxisymFunctionBase& fnc, const AxisymIntLimits& lim)
{
...
#ifdef SIN2
    math::ScaledIntegrand<math::ScalingSin2>
		    transf_l(math::ScalingSin2(lim.lambda_min, lim.lambda_max), integrand);
    math::ScaledIntegrand<math::ScalingCos2>
		    transf_n(math::ScalingCos2(lim.nu_min,     lim.nu_max),     integrand);
    math::ScaledIntegrand<math::ScalingSin2Inv>
		    transf_lI(math::ScalingSin2Inv(fnc.point.coordsys.Delta2, lim.lambda_min, lim.lambda_max), integrand);
    math::ScaledIntegrand<math::ScalingCos2Inv>
		    transf_nI(math::ScalingCos2Inv(fnc.point.coordsys.Delta2, lim.nu_min,     lim.nu_max),     integrand);
#else
    ...
    der.dJrdE = (lim.lambda_min==lim.lambda_max ? 
        ...
        math::integrateGL(transf_l, 0, 1, lim.l_integrOrder) ) / (4*M_PI);
    der.dJzdE = math::integrateGL(transf_n, 0, 1, lim.n_integrOrder) / (2*M_PI);
    //printitS(transf_l,0,1); printitC(transf_n,0,1);
    ...
    der.dJrdI3 = - (lim.lambda_min==lim.lambda_max ? 
        integrand.limitingIntegralValue(lim.lambda_min) :
        math::integrateGL(transf_l, 0, 1, lim.l_integrOrder) ) / (4*M_PI);
    der.dJzdI3 = -math::integrateGL(transf_n, 0, 1, lim.n_integrOrder) / (2*M_PI);
    //printitS(transf_l,0,1); printitC(transf_n,0,1);
    ...
    der.dJrdLz = -fnc.Lz * (lim.lambda_min==lim.lambda_max ? 
        integrand.limitingIntegralValue(lim.lambda_min) :
        math::integrateGL(transf_lI, 0, 1, lim.l_integrOrder) ) / (4*M_PI);
    der.dJzdLz = -fnc.Lz * math::integrateGL(transf_nI, 0, 1, lim.n_integrOrder) / (2*M_PI);
    //printitSI(transf_lI,0,1); printitCI(transf_nI,0,1);
    return der;
}
    ...
    const AxisymFunctionBase& fnc, const AxisymIntLimits& lim)
{
    ...
    double det1 = (dJ.dJrdI3 * dJ.dJzdLz - dJ.dJrdLz * dJ.dJzdI3);
    double det2 = -(dJ.dJrdE * dJ.dJzdLz - dJ.dJrdLz * dJ.dJzdE);
    if(lim.nu_min==lim.nu_max || det==0) {
        ...
        der.Omegaphi = det1 / det;  // dE/dJphi
        der.dI3dJr   =-dJ.dJzdE / det;
        der.dI3dJz   = dJ.dJrdE / det;
        der.dI3dJphi = det2 / det;
    }
    der.dLzdJr  = 0;
    der.dLzdJz  = 0;
    der.dLzdJphi= 1;
    ...
    const AxisymFunctionBase& fnc, const AxisymIntLimits& lim)
{
    ...
    const double ylI = math::scale(transf_lI.scaling, fnc.point.lambda);
    const double yn = lim.nu_min==lim.nu_max ? 0 : math::scale(transf_n.scaling, fabs(fnc.point.nu));
    const double ynI = lim.nu_min==lim.nu_max ? 0 : math::scale(transf_nI.scaling, fabs(fnc.point.nu));
    integrand.n = AxisymIntegrand::nminus1;  // momentum goes into the denominator
    // derivatives w.r.t. E
    integrand.a = AxisymIntegrand::aminus1;
    integrand.c = AxisymIntegrand::czero;
    der.dSdE =
        signldot * math::integrateGL(transf_l, 0, yl, lim.l_integrOrder) / 4
      + signndot * math::integrateGL(transf_n, 0, yn, lim.n_integrOrder) / 4;
    //printitS(transf_l,0,yl); printitC(transf_n,0,yn);
    // derivatives w.r.t. I3
    integrand.a = AxisymIntegrand::aminus1;
    integrand.c = AxisymIntegrand::cminus1;
    der.dSdI3 = 
    ...
    der.dSdLz = fnc.Lz==0 ? 0 : fnc.point.phi +
        signldot * -fnc.Lz * math::integrateGL(transf_lI, 0, ylI, lim.l_integrOrder) / 4
      + signndot * -fnc.Lz * math::integrateGL(transf_nI, 0, ynI, lim.n_integrOrder) / 4;
    //printitSI(transf_lI,0,ylI); printitCI(transf_nI,0,ynI);
    return der;
}
...
EXP Actions computeActions(const AxisymFunctionBase& fnc, const AxisymIntLimits& lim)
{
    Actions acts;
    AxisymIntegrand integrand(fnc);
    math::ScaledIntegrand<math::ScalingCub>
    ...
    integrand.c = AxisymIntegrand::czero;
    acts.Jr = math::integrateGL(transf_l, 0, 1, lim.l_integrOrder) / M_PI;
    // factor of 2 in Jz because we only integrate over half of the orbit (z>=0)
    acts.Jz = math::integrateGL(transf_n, 0, 1, lim.n_integrOrder) / M_PI * 2;
    acts.Jphi = fnc.Lz;
    return acts;
}
    ...
    const coord::PosVelCyl& point)
{
    ...
    const AxisymIntLimits lim = findIntegrationLimitsAxisym(fnc, NAN);
...
EXP Actions actionsAxisymFudge(const potential::BasePotential& potential,
    const coord::PosVelCyl& point, double focalDistance, double lambda_dip)
{
    ...
    if(!isFinite(fnc.E+fnc.I3+fnc.Lz) || fnc.E>=0)
        return Actions(NAN, NAN, fnc.Lz);
    const AxisymIntLimits lim = findIntegrationLimitsAxisym(fnc, lambda_dip);
    return computeActions(fnc, lim);
}

EXP ActionAngles actionAnglesAxisymFudge(const potential::BasePotential& potential, 
    const coord::PosVelCyl& point, double focalDistance, double lambda_dip, Frequencies* freq)
{
    if(!isAxisymmetric(potential))
        throw std::invalid_argument("Fudge approximation only works for axisymmetric potentials");
    if(focalDistance<=0)
...
/** compute the best-suitable focal distance at a 3d grid in E, L/Lcirc(E) and I/Imax*/
void createGridFocalDistance(
			     const potential::PtrPotential pot,
			     const std::vector<double>& gridE, const std::vector<double>& gridL,
			     const std::vector<double>& gridI,
			            /*output: focal distance*/ std::vector<double> &grid3dD,
                                  /*output: focal lambda dip*/ std::vector<double> &grid3dDip,
    /*output: radius of shell orbit normalized to R_circ(E) */ math::Matrix<double>& grid2dR)
{
	int sizeE = gridE.size(), sizeL = gridL.size(), sizeI = gridI.size(),
	sizeEL = (sizeE-1) * (sizeL-1);
    ...
    // loop over the grid in E and L (combined index for better load
    // balancing)
	const double Ibot=0.35, Itop=0.6, DI = Itop-Ibot;
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic)
#endif
	for(int iEL = 0; iEL < sizeEL; iEL++) {
		try{
			int iE    = iEL / (sizeL-1);//miss out highest E
			int iL    = iEL % (sizeL-1);//miss out hghest L
			double E  = gridE[iE];
			double Rc = R_circ(*pot, E);
			double vc = v_circ(*pot, Rc);
			double Lc = Rc * vc;
			double Lz = Lc * gridL[iL];
			double Rsh, Delta_shell  = estimateFocalDistanceShellOrbit(*pot, E, Lz, &Rsh);
			grid2dR(iE, iL) = Rsh / Rc;
			double lambda = pow_2(Rsh) + pow_2(Delta_shell);
			double vmer2 = (2*(E - pot->value(coord::PosCyl(Rsh,0,0))) - pow_2(Lz/Rsh));
			double I3max = .5 * lambda * vmer2;
			for(int iI = 0; iI < sizeI; iI++){
				int index = (iE * sizeL + iL) * sizeI + iI;
				double vR = sqrt(fmax(0,vmer2 - 2*gridI[iI]*I3max/lambda));
				FDfinder state(E,Rsh,vR,Delta_shell,pot);
				//Include factor 0.98 in Delta_excl to
				//compensate for interpolation errors
				double lambda_dip, Delta_excl = fmin(Delta_shell, .98*state.bestFD(&lambda_dip));
				grid3dDip[index] = lambda_dip;
				double Dhat = .5*(Delta_shell - Delta_excl) * 2/(exp(4*(Lz*Lz/I3max)) + 1);
				double Dbar = Delta_shell - Dhat;
				if(gridI[iI] > Itop) grid3dD[index] = Dbar - Dhat;
				else if(gridI[iI]>Ibot){
					double theta = M_PI * (Itop - gridI[iI]) / DI;
					grid3dD[index] = Dbar - Dhat * cos(theta);
				}
				else grid3dD[index] = Delta_shell;
//				grid3dD[index] = Delta_shell;//To show this doesn't work
			}
		}
	...
	for(int iE=0; iE<sizeE-1; iE++) {
		...
		for(int iI = 0; iI < sizeI; iI++){
			grid3dD[iE*sizeL*sizeI + iI] =
				grid3dD[(iE*sizeL + 1)*sizeI + iI];  // for Lz=0 don't trust the focal distance from shell orbit//Jphi=Lc so Imax=0
			grid3dD[(iE*sizeL + (sizeL-1))*sizeI + iI] =
				grid3dD[(iE*sizeL + (sizeL-2))*sizeI + iI]; //Jphi=Lz so I3max=0 orbit in plane & circular
			grid3dDip[iE*sizeL*sizeI + iI] =
				grid3dDip[(iE*sizeL + 1)*sizeI + iI];  // for Lz=0 don't trust the focal distance from shell orbit//Jphi=Lc so Imax=0
			grid3dDip[(iE*sizeL + (sizeL-1))*sizeI + iI] =
				grid3dDip[(iE*sizeL + (sizeL-2))*sizeI + iI]; //Jphi=Lz so I3max=0 orbit in plane & circular
		}
	}
	...
	for(int iL=0; iL<sizeL; iL++) {
		...
		for(int iI=0; iI<sizeI; iI++) grid3dD[((sizeE-1)*sizeL + iL)*sizeI + iI] = 0.;
		for(int iI=0; iI<sizeI; iI++) grid3dDip[((sizeE-1)*sizeL + iL)*sizeI + iI] = 10;
	}
    ...
    invPhi0(1./_pot->value(coord::PosCyl(0,0,0))), pot(_pot), interp(*pot)
{
    ...
    std::vector<double> grid3dD(sizeE * sizeL * sizeI);  // focal distance
    std::vector<double> grid3dDip(sizeE * sizeL * sizeI);  // lambda_dip
    math::Matrix<double> grid2dR;  // Rshell / Rcirc(E)
    createGridFocalDistance(pot, gridE, gridL, gridI, /*output*/ grid3dD, grid3dDip, grid2dR);
    interpD   = math::LinearInterpolator3d(gridEscaled, gridLscaled, gridIscaled, grid3dD); //, /*regularize*/true);
    interpDip = math::LinearInterpolator3d(gridEscaled, gridLscaled, gridIscaled, grid3dDip); //, /*regularize*/true);
		    ...
		    for(int iE=0; iE<sizeE; iE++) {
			    for(int iL=0; iL<sizeL; iL++) {
				    strm <<
						    ...
						    utils::pp(gridL[iL],       6) +' ' +
						    "0     \t"+
						    utils::pp(grid3dD[(iE*sizeL + iL)*sizeI + sizeI/2], 7) +'\t'+
...
#pragma omp parallel for schedule(dynamic)
    ...
    for(int iEL=0; iEL<sizeEL; iEL++) {
	    ...
	    // focal distance: presently can't work when fd=0 exactly
		    double fd   = fmax(grid3dD[(iE*sizeL + iL)*sizeI + sizeI-1], Rc * 1e-4);
		    double lambda_dip = grid3dDip[(iE*sizeL + iL)*sizeI + sizeI-1];
	    ...
	    // explore the range of I3
		    for(int iI=0; iI<sizeI; iI++) {
			    ...
			    AxisymIntLimits lim = findIntegrationLimitsAxisym(fnc, lambda_dip);
		    ...
		    for(int iL=0; iL<sizeL; iL++) {
			    for(int iI=0; iI<sizeI; iI++) {
				    strm <<
						    ...
						    utils::pp(gridL[iL],       6) +' ' +
						    utils::pp(gridI[iI],       6) +'\t'+
						    utils::pp(grid3dD [ (iE * sizeL + iL) * sizeI + iI], 7) +'\t'+
						    utils::pp(grid2dR(iE, iL), 7) +'\t'+
...
EXP Actions ActionFinderAxisymFudge::actions(const coord::PosVelCyl& point) const
{
	...
	double fd    = fmax(0, interpD.value(xi, chi, .9));   // trial focal distance
	...
	fd    = fmax(0, interpD.value(xi, chi, psi));   // final focal distance
	double lam_dip = interpDip.value(xi, chi, psi); 

    // if we are not using the 3d interpolation, then compute the actions by the direct method
	if(intJr.empty())
		return actionsAxisymFudge(*pot, point, fd, lam_dip);
   ...
   (const coord::PosVelCyl& point, Frequencies* freq) const
{
	...
	double psi   = math::scale(scaling, math::clip(I3 / I3max, 0., 1.));
	fd    = fmax(0, interpD.value(xi, chi, psi));   // final focal distance
	double lambda_dip = interpDip.value(xi, chi, psi); 

	return actionAnglesAxisymFudge(*pot, point, fd, lambda_dip, freq);
}
...
EXP double ActionFinderAxisymFudge::focalDistance(const coord::PosVelCyl& point) const
{
    ...
    return fmax(0, interpD.value(xi, chi, .9));//preliminary value only
}
