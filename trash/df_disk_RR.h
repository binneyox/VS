...
double
    beta_r, beta_z,  ///< factors describing the growth of velocity dispersion with age
...
AgeVelocityDispersionParam() :  ///< set default values disabling any variation
    beta_r(0), beta_z(0), Tsfr(INFINITY), sigmabirth(1) {}
};
...
/** Distribution function for quasi-isothermal disk, similar but somewhat different from the one
    ...
    In the case of a single population (beta_i=0 or sigmabirth=1), it is given by
...
*/
class EXP QuasiIsothermal: public BaseDistributionFunction{
...
/** Another type of disk distribution function, which resembles QuasiIsothermal and produces
    ...
    \f$  f_r    = Jvel^p / J_{r,0}^(1+p) \exp( - J_r Jvel^p / J_{r,0}^(1+p) )    \f$,
    \f$  f_r    = Jvel^p / J_{z,0}^(1+p) \exp( - J_z Jvel^p / J_{z,0}^(1+p) )    \f$,
...
*/
class EXP Exponential: public df::BaseDistributionFunction{
...
/// Parameters that describe the new_exp distribution function
struct newExpParam: public AgeVelocityDispersionParam{
	double
			norm,       ///< overall normalization factor with the dimension of mass (NOT the actual mass)
			Jr0,        ///< scale action setting the radial velocity dispersion
			Jz0,        ///< scale action setting the disk thickness and the vertical velocity dispersion
			Jcut,	    ///< scale Jphi within which DF vanishes as Jphi -> 0
			Jphi0,      ///< scale action setting the disk radius
			Js,	    ///< de-dimensionalising action (eg of Sun)
			addJden,    ///< additional contribution to the sum of actions that affects the density profile
			power,	    ///< power of radial variation of dispersions
			addJvel,    ///< same for the part that affects the velocity dispersion profiles
			coefJr,     ///< weight of radial action in the sum of actions
			coefJz;     ///< same for the vertical action
	newExpParam() :  ///< set default values for all fields
	    norm(NAN), Jr0(NAN), Jz0(NAN), Jcut(NAN), Jphi0(NAN), Js(1), addJden(0), power(1), addJvel(0), coefJr(1.0), coefJz(0.25) {}
};

/** The Exponential DF modified so it -> 0 as Jphi ->0.
    ...
    Generalization to the case of age--velocity dispersion relation is similar to QuasiIsothermal.
*/
class EXP newExp: public df::BaseDistributionFunction{
	const newExpParam par;     ///< parameters of the DF
	public:
		newExp(const newExpParam& params);
		virtual double value(const actions::Actions &J) const;
		void write_params(std::ofstream &strm,const units::InternalUnits &intUnits) const;
};

/// Parameters that describe the basic_exp distribution function
struct basicExpParam{
	double
			norm,       ///< overall normalization factor with the dimension of mass (NOT the actual mass)
			Jr0,        ///< scale action setting the radial velocity dispersion
			Jz0,        ///< scale action setting the disk thickness and the vertical velocity dispersion
			...
			pz,	    ///< power of radial variation of sigz
			addJvel;    ///< same for the part that affects the velocity dispersion profiles
	basicExpParam() : ///< set default values for all fields
	    norm(NAN), Jr0(NAN), Jz0(NAN), Jcut(NAN), Jphi0(NAN), addJden(0), pr(0.5), pz(0.5), addJvel(0) {}
};

/// Parameters that describe the basic_exp distribution function: same
/// as newExp except it's intended for a coeval cpt so has no age-V relation

class EXP basicExp: public df::BaseDistributionFunction{
	const basicExpParam par;     ///< parameters of the DF
	public:
		basicExp(const basicExpParam& params);
		virtual double value(const actions::Actions &J) const;
		void write_params(std::ofstream &strm,const units::InternalUnits &intUnits) const;
};
