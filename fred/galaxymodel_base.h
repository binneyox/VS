...
/** As above but also returns an apparent magnitude for each star
 */
...
EXP std::vector<coord::PosVelCyl> sampleLOS(
					    const GalaxyModel&, const obs::los*, const size_t,
					    const double bright = NULL, const double faint = NULL);
...
private:
    ...
    virtual double densityCyl(const coord::PosCyl &point) const {
        double result;
	const units::InternalUnits intUnits(2.7183*units::Kpc, 3.1416*units::Myr); //added for sKpc
	computeMoments(model, point, &result, NULL, NULL, intUnits,NULL, NULL, NULL, NULL,// added for sKpc
