...
class EXP CompositeDF: public BaseDistributionFunction{
public:
...
/*    virtual std::vector<double> values(const actions::Actions &J) const {
	    std::vector<double> vals;
	    for(unsigned int i=0; i<components.size(); i++)
		    vals[i] = components[i]->value(J);
	    return vals;
    }*/
    virtual void write_params(std::ofstream &strm,const units::InternalUnits &intUnits) const {
	    for(unsigned int i=0; i<components.size(); i++){
		    components[i]->write_params(strm,intUnits);
		    strm << '\n';
	    }
