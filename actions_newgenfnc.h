...
class EXP GenFncFrac {
	public:
		...
		GenFncFrac(GenFncFitFrac& frac, double* params, Actions* derivs):
		    mz(frac.mz), mphi(frac.mphi), B(params[0]), b(tanh(params[1])), onembsq(1-b*b),
			dBdJ(derivs[0]), dpsidJ(derivs[1]) {}
		GenFncFrac(){}
		GenFncFrac(double _B, double _b, Actions _dBdJ, Actions _dpsidJ, int _mz, int _mphi=0):
		    B(_B), b(_b), dBdJ(_dBdJ), dpsidJ(_dpsidJ), onembsq(1-b*b), mz(_mz), mphi(_mphi){}
		//contrib to JT
...
class EXP GenFncFitSeries{
    ...
    /** construct the object for the fixed indexing scheme,
		...
		GenFncFitSeries(const GenFncIndices& indices, const GenFncFitFracs& fracs,
				const Actions& acts);
    ...
    */
		...
		GenFncIndices expand(std::vector<double>& params, GenFncFitFracs& fracs);
