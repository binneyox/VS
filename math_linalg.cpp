...
template<> EXP void blas_daxpy(double alpha, const std::vector<double>& X, std::vector<double>& Y)
{
...
template<> EXP void blas_dmul(double alpha, std::vector<float>& Y)
{
...
template<> EXP void blas_dmul(double alpha, std::vector<double>& Y)
{
    const size_t size = Y.size();
    for(size_t i=0; i<size; i++)
        Y[i] *= alpha;
...
template<>
EXP double blas_dnrm2(const std::vector<double>& X)
{
    double result = 0;
...
template<>
EXP double blas_dnrm2(const Matrix<double>& X)
{
    double result = 0;
...
template<>
EXP void blas_daxpy(double alpha, const BandMatrix<double>& X, BandMatrix<double>& Y)
{
...
template<> EXP void blas_dmul(double alpha, BandMatrix<double>& Y)
{
...
template<>
EXP void blas_dgemv(CBLAS_TRANSPOSE Trans, double alpha, const BandMatrix<double>& mat,
...
EXP std::vector<double> solveBand(const BandMatrix<double>& mat, const std::vector<double>& rhs)
{
    const ptrdiff_t size = mat.rows(), band = mat.bandwidth(), width = band * 2 + 1;
...
EXP void blas_daxpy(double alpha, const MatrixType& X, MatrixType& Y)
{
    if(alpha==1)
        mat(Y) += mat(X);
    else if(alpha!=0)
...
template<typename MatrixType>
EXP void blas_dmul(double alpha, MatrixType& Y)
{
    mat(Y) *= alpha;
}

template<typename MatrixType>
EXP void blas_dgemv(CBLAS_TRANSPOSE TransA, double alpha, const MatrixType& A,
...
template EXP void blas_daxpy(double, const Matrix<double>&, Matrix<double>&);
template EXP void blas_daxpy(double, const SparseMatrix<double>&, SparseMatrix<double>&);
template EXP void blas_dmul(double, Matrix<double>&);
template EXP void blas_dmul(double, SparseMatrix<double>&);
template EXP void blas_dgemv(CBLAS_TRANSPOSE, double, const Matrix<double>&,
    const std::vector<double>&, double, std::vector<double>&);
template EXP void blas_dgemv(CBLAS_TRANSPOSE, double, const SparseMatrix<double>&,
    const std::vector<double>&, double, std::vector<double>&);
template EXP void blas_dgemm(CBLAS_TRANSPOSE, CBLAS_TRANSPOSE,
...
EXP LUDecomp::LUDecomp(const Matrix<double>& M) :
    sparse(false), impl(new LUDecompImpl(mat(M))) {}

EXP LUDecomp::LUDecomp(const SparseMatrix<double>& M) :
...
EXP LUDecomp::LUDecomp(const LUDecomp& src) :
...
EXP std::vector<double> LUDecomp::solve(const std::vector<double>& rhs) const
{
    if(!impl)
...
EXP CholeskyDecomp::CholeskyDecomp(const Matrix<double>& M) :
...
EXP CholeskyDecomp::~CholeskyDecomp() { delete static_cast<const CholeskyDecompImpl*>(impl); }

EXP CholeskyDecomp::CholeskyDecomp(const CholeskyDecomp& src) :
    impl(new CholeskyDecompImpl(*static_cast<const CholeskyDecompImpl*>(src.impl))) {}

EXP Matrix<double> CholeskyDecomp::L() const { 
    if(!impl)
...
EXP std::vector<double> CholeskyDecomp::solve(const std::vector<double>& rhs) const {
    if(!impl)
        throw std::runtime_error("CholeskyDecomp not initialized");
...
EXP SVDecomp::SVDecomp(const Matrix<double>& M) :
    impl(new SVDecompImpl(mat(M), Eigen::ComputeThinU | Eigen::ComputeThinV)) {}

EXP SVDecomp::~SVDecomp() { delete static_cast<SVDecompImpl*>(impl); }

EXP SVDecomp::SVDecomp(const SVDecomp& src) :
    impl(new SVDecompImpl(*static_cast<const SVDecompImpl*>(src.impl))) {}

EXP Matrix<double> SVDecomp::U() const { 
    if(!impl)
...
EXP Matrix<double> SVDecomp::V() const { 
    if(!impl)
        throw std::runtime_error("SVDecomp not initialized");
...
EXP std::vector<double> SVDecomp::S() const { 
    if(!impl)
        throw std::runtime_error("SVDecomp not initialized");
    return toStdVector(static_cast<const SVDecompImpl*>(impl)->singularValues());
}

EXP std::vector<double> SVDecomp::solve(const std::vector<double>& rhs) const {
    if(!impl)
        throw std::runtime_error("SVDecomp not initialized");
...
template<> EXP void blas_daxpy(double alpha, const Matrix<double>& X, Matrix<double>& Y)
{
...
template<> EXP void blas_daxpy(double alpha, const SparseMatrix<double>& X, SparseMatrix<double>& Y)
{
    if(X.rows() != Y.rows() || X.cols() != Y.cols())
        throw std::length_error("blas_daxpy: incompatible sizes of input arrays");
    if(alpha==0) return;
...
template<> EXP void blas_dgemv(CBLAS_TRANSPOSE TransA, double alpha, const Matrix<double>& A,
    const std::vector<double>& X, double beta, std::vector<double>& Y)
{
    gsl_blas_dgemv((CBLAS_TRANSPOSE_t)TransA, alpha, MatC(A), VecC(X), beta, Vec(Y));
}

template<> EXP void blas_dgemv(CBLAS_TRANSPOSE TransA, double alpha, const SparseMatrix<double>& A,
    const std::vector<double>& X, double beta, std::vector<double>& Y)
{
    if(A.impl == NULL)
...
template<> EXP void blas_dgemm(CBLAS_TRANSPOSE TransA, CBLAS_TRANSPOSE TransB,
