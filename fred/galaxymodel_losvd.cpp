...
#include "utils.h"
#include "units.h" //added for sKpc
#include <cmath>
#include <stdexcept>
#include <cassert>
...
template<int N>
EXP TargetLOSVD<N>::TargetLOSVD(const LOSVDParams& params) :
...
template<int N>
EXP void TargetLOSVD<N>::addPoint(const double point[6], double _mult, double* datacube) const
{
...
template<int N>
EXP void TargetLOSVD<N>::finalizeDatacube(math::Matrix<double> &datacube, StorageNumT* output) const
{
...
template<int N>
EXP void TargetLOSVD<N>::computeDFProjection(const GalaxyModel& model, StorageNumT* output) const
{
...
template<int N>
EXP std::vector<double> TargetLOSVD<N>::computeDensityProjection(const potential::BaseDensity& density) const
{
...
template<int N>
EXP std::string TargetLOSVD<N>::coefName(unsigned int index) const
{
    if(index >= numCoefs())
...
template<> EXP const char* TargetLOSVD<0>::name() const { return "LOSVD0"; }
template<> EXP const char* TargetLOSVD<1>::name() const { return "LOSVD1"; }
template<> EXP const char* TargetLOSVD<2>::name() const { return "LOSVD2"; }
template<> EXP const char* TargetLOSVD<3>::name() const { return "LOSVD3"; }

// template instantiations
template EXP class TargetLOSVD<0>;
template EXP class TargetLOSVD<1>;
template EXP class TargetLOSVD<2>;
template EXP class TargetLOSVD<3>;


//----- TargetKinemShell -----//

template<int N>
EXP std::string TargetKinemShell<N>::coefName(unsigned int index) const
{
    return (index < bspl.numValues() ? "Vr[" : "Vt[") + utils::toString(index % bspl.numValues()) + "]";
}

template<int N>
EXP void TargetKinemShell<N>::addPoint(const double point[6], double mult, double output[]) const
{
...
template<int N>
EXP void TargetKinemShell<N>::computeDFProjection(const GalaxyModel& model, StorageNumT* output,const units::InternalUnits& intUnits) const //sKpc
{
...
#pragma omp parallel for schedule(static)
    ...
    for(int n=0; n<size; n++) {
        ...
        double dens;
        coord::Vel2Cyl vel2;
        computeMoments(model, coord::PosCyl(r,0,0), &dens, NULL, &vel2, intUnits); //added for sKpc
...
template<int N>
EXP std::vector<double> TargetKinemShell<N>::computeDensityProjection(const potential::BaseDensity&) const
{
    // this operation does not make sense
    throw std::runtime_error("TargetKinemShell cannot be applied to a density object");
}

template<> EXP const char* TargetKinemShell<0>::name() const { return "KinemShell0"; }
template<> EXP const char* TargetKinemShell<1>::name() const { return "KinemShell1"; }
template<> EXP const char* TargetKinemShell<2>::name() const { return "KinemShell2"; }
template<> EXP const char* TargetKinemShell<3>::name() const { return "KinemShell3"; }

// template instantiations
template EXP class TargetKinemShell<0>;
template EXP class TargetKinemShell<1>;
template EXP class TargetKinemShell<2>;
template EXP class TargetKinemShell<3>;
